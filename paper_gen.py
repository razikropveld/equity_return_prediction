...
<div class="abstract-box">
  ...
  <p style="margin-top:8px;">
    ...
    Using a point-in-time fundamental database covering over 5,700 U.S. equities, an XGBoost model is
    evaluated on a strict holdout period from 2012 to 2023&mdash;spanning the post-crisis bull market,
    the COVID-19 shock, and the 2022 rate cycle. A custom three-block TSCV design places the validation
    block <em>prior</em> to the training window, allowing the model to adapt more fluidly to changing
    market regimes. Applied to a long-only top-30 strategy, the framework produces annualized returns of
    approximately 14% versus 7% for the equal-weighted benchmark, outperforming in 61% of individual
    periods. The signal is broad rather than concentrated: the lowest-ranked decile consistently
    underperforms while strong returns extend well beyond the top 30 stocks&mdash;suggesting genuine
    cross-sectional discrimination rather than overfitting.
  </p>
...
<p>
  ...
  metrics are strategy returns and their statistical robustness&mdash;not RMSE or Sharpe ratios
