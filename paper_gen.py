...
<p>
  The key design choice is the placement of the validation block before rather than after the
  training window. The rationale is empirical: <code>num_boost_rounds</code>&mdash;the hyperparameter
  controlling tree count, selected via early stopping on the validation block&mdash;was found to vary
  considerably less across test periods than the model parameters learned from the training data.
  Because the learned parameters are far more sensitive to the local market regime, it is more
  important that the training window be temporally close to the test period than the validation block.
  Placing validation earlier satisfies this priority without sacrificing the ability to tune
  model complexity.
</p>
